...
import { Controls } from "@iea/react-components";
...
const BubbleWrapper = () => {
  ...
  for (let i in colors) {
    data.push({
      ...
      name:
        Number(i) < 5
          ? `${range[Number(i)]} - ${range[Number(i) + 1]}`
          : `> ${range[Number(i)]}`,
    });
  }
  ...
  function filterData(d) {
    let hasRadius = d.z !== 0;
    let isNotNull =
      d.x !== null && d.y !== null && d.z !== null && d.color !== null;
