...
export const getCountryNameByISO = (iso) =>
  !countries.find((d) => d.ISO3 === iso)
    ? null
    : countries.find((d) => d.ISO3 === iso).region;
export const getCentroidLabelByISO = (_) => ["to-string", ["get", "region"]];
export const getMonthString = (date) =>
  new Date(1, date - 1, 1).toLocaleString("default", { month: "long" });
export const useIntervalLogic = (entries, interval) =>
  entries[["year", "month", "day"].findIndex((d) => d === interval)];
export const disputedRegionsISO = [
  "ABCDE",
  "ABCD",
  "VAT",
  "SMR",
...
export function colorsByVariables(
  selector,
  countries,
  colType,
  valueType,
  ...
  let tempColors = !colorArray[colType]
    ? colorArray.default
    : colorArray[colType];
  ...
  for (let i = 1; i < tempColors.length - 1; i++)
    valueRange.push(pivotValue * i + minMax[0]);
  ...
  let contScale = scaleLinear().domain(range).range(colorRange);
...
export function getTerritoryMinMax(minmax, type, group) {
  ...
  if (type === "anomaly") {
    let [min, max] = getAnomalyMinMax([ceil(minmax[0]), floor(minmax[1])]);
    result = [min, "0", max];
    if (isPrecip) {
...
export function getGridMinMax(minmaxValues, type, mapType, idx, group, id) {
  ...
  if (isAnomaly) {
    if (isPrecip && isEvap)
      minmax = getAnomalyMinMax([fParseTwo(minmax[0]), fParseTwo(minmax[1])]);
    ...
    if (isPrecip && isEvap)
      minmax = [minmax[0].toFixed(2), minmax[1].toFixed(2)];
    else if (isPrecip && !isEvap)
      minmax = [minmax[0].toFixed(1), `≥ ${minmax[1].toFixed(1)}`];
    else if (isWind) minmax = [ceil(minmax[0]), `≥ ${floor(minmax[1])}`];
    else minmax = [ceil(minmax[0]), floor(minmax[1])];
  }

  return isAnomaly ? [minmax[0], 0, minmax[1]] : minmax;
}
...
export function getPopupInfo(countries, selected, unit, decimal) {
  ...
  if (selected && selectedCountry)
    return `${selectedCountry.name}</br><b>${selectedCountry.value.toFixed(
      decimal
    )} ${unit}</b>`;
