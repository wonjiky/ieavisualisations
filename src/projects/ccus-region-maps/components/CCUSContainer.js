...
import { useMap, Map } from "@iea/react-components";

const CCUSContainer =  ({ data, regions, toggle }) => {
  const config = {
    map: "custom",
  ...
  React.useEffect(() => {
    ...
    const {
      types,
      minMax,
      "Oil and gas reservoirs": reservoirs,
      hubs,
      ...rest
    } = data;

    for (let reservoir in reservoirs) {
      map.addSource(`${regions.region}-Reservoir-${reservoir}`, {
        type: "vector",
        url: reservoirs[reservoir].url,
      });
    }
    for (let source in rest) {
      map.addSource(`${regions.region}-${source}`, {
        type: "geojson",
        data: rest[source],
      });
    }
    ...
    map
      .addLayer({
        ...
        paint: {
          ...
          "heatmap-radius": ["interpolate", ["linear"], ["zoom"], 2, 5, 4, 25],
