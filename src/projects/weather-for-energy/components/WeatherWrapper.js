...
export default function WeatherWrapper() {
  ...
  let countryQuery = intervalLogic(
    [
  ...
  let downloadQuery = intervalLogic(
    [
  ...
  let downloadButtonLabel = intervalLogic(
    [
  ...
  useEffect(() => {
    if (mapType === "grid" || selectedCountries.length === 0) return;
  ...
  }, [selectedCountries, countrQueryString, viewInterval, decimal, mapType]);
  ...
  const controls = {
    topleft: [
      ...
      },
      {
        ...
        value:
          mapType === "territory"
            ? intervalLogic([year, month, day], viewInterval)
            : viewInterval === "month"
            ? gridTime.month
            : gridTime.year,
        min:
          mapType === "territory"
            ? intervalLogic([minYear, minMonth, minDay], viewInterval)
            : viewInterval === "month"
            ? minMonth
            : minYear,
        max:
